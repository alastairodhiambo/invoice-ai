...
def get_vendor_name(image_path):
    ...
    vendor_paths = glob.glob(os.path.join("data", "*"))
    for vendor_path in vendor_paths:
        vendor_dir = os.path.join(vendor_path, "images", "*")
        vendor_files = glob.glob(vendor_dir)
        # for vendor_file in vendor_files:
        scores.append(image_resnet_score(image_path, vendor_files[0]))
...
def parse_data(vendor_name):
    text_dir = os.path.join("data", vendor_name, "text", "*")
...
def parse_annotations(vendor_name):
    anno_dir = os.path.join("data", vendor_name, "annotations", "*")
...
def construct_prompt(texts, annotations, fields, test_text):
    prompts = []
    separator = "\n============\n"

    # generate the example text to follow the pattern of.
    ...
    # Add the test text to the prompt
    return separator.join(prompts) + "Print out the json output for the data below as has been done in the above examples:\n\n" + test_text

# f"\n\nReplace null with the correct extracted data." + f"\n\n{test_json}"
